...
#define RES_MATRIX "test_matrix.txt" // original : maxtrix.txt 
#define TRANSITIVE "transitive_"
#define DOT_TXT ".txt"
#define THREAD_INPUT 10
...
void matrix_setting(FILE *stream, int *A);
...
int main(int argc, char *argv[])
{
  ...
  matrix_setting(stream,R_1);
 
  if((stream_2 = fopen(RES_MATRIX, "r")) == NULL){
    fprintf(stderr, "resource matrix file open failed.(%s)\n", RES_MATRIX);
  }
  matrix_setting(stream_2, R_2);
  ...
  if(file_writing(stream_3, depth, dim, Product)){
