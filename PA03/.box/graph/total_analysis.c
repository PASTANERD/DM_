...
static Node *queue_temp[TOTAL_SIZE];
static Node *node;
static Edge *edge;
static int group;
static int distance;
...
void testprint(Node *node, Edge *edge){
  stream = fopen(TOTAL_EDGE, "w");
  printf("printing results now.\n"); 
  /*
  fprintf(stream, "test printf for all nodes\n");
  
  for(int i = 0; i < node_size ; i++)
    fprintf(stream ,"%d)\t id = %d, \tpartition = %d\n", i , node[i].id, node[i].partition);

  fprintf(stream, "test printf for all edges\n");
  for(int i = 0 ; i < edge_size ; i ++){
    fprintf(stream,"X = %d\tY = %d\tdistance = %d\n", edge[i].X->id, edge[i].Y->id, edge[i].weight);
  }
  */
/*
